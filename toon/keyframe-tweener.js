            ...
            for (i = 0, maxI = sprites.length; i < maxI; i += 1) {
                for (j = 0, maxJ = sprites[i].keyframes.length - 1; j < maxJ; j += 1) {
                    ...
                    if ((sprites[i].keyframes[j].frame <= currentFrame) &&
                        ...
                        // JD: 6
                        if (startKeyframe.options) {
                            for (var opt in startKeyframe.options) {
                                optionsDistance[opt] = endKeyframe.options[opt] - startKeyframe.options[opt];
                                extras[opt] = ease(currentTweenFrame, startKeyframe.options[opt], optionsDistance[opt], duration);  
                            }
                        }
                        ...
                        // Draw the sprite.
                        if (startKeyframe.options) {
                            sprites[i].draw(renderingContext, extras);
                        } else {
